...
- **Phase 5 — Roles Expansion** 🟡 *(in progress — contractor projects done, supplier dashboard and admin panel not started)*: registration is now a 3-way Customer/Contractor/Supplier tab. Contractors get a "My Projects" page (create/list/detail), a dashboard summary, and an optional project-link dropdown at checkout — spend and budget-remaining are computed live off linked orders. Still open: **supplier dashboard** (listings/inventory management, order fulfillment — this is also what unlocks replacing the `orders:update-status` artisan command and building real `shipments`/`shipment_events` tracking from Phase 4), and **Filament admin panel** (supplier verification queue, catalog moderation, order oversight). 10 new feature tests, 66/66 passing.
